...
docker exec sad_mysql mysql --local-infile -uroot -padmin OLTP -e "LOAD DATA LOCAL INFILE '/data/product_category_name_translation.csv' INTO TABLE OLTP_CATEGORIES_TRANSLATION FIELDS TERMINATED BY ',' LINES TERMINATED BY '\r\n' IGNORE 1 ROWS (product_category_name, product_category_name_english)"
